...
void ra_spinr_nx_kernel(
    ...
    for (int t = 0; t < n_samples; ++t) {
        // State rotation: R * s
        int re_0 = clip_ls(static_cast<int>(R[R_idx + 0] * s[0]));
        int re_1 = clip_ls(static_cast<int>(R[R_idx + 1] * s[1]));
        int im_0 = clip_ls(static_cast<int>(R[R_idx + 1] * s[0]));
        int im_1 = clip_ls(static_cast<int>(R[R_idx + 0] * s[1]));

        s[0] = clip_ls(re_0 - re_1); // not accurate since in ucode "-" could overflow
        ...
        // Add to state
        s[0] = clip_ls(s[0] + inp[0]);
        s[1] = clip_ls(s[1] + inp[1]);
        ...
        if (t > t_grd) {
            ...
            gradient = static_cast<int>(beta_grd * gradient) + 
                       clip_ls(static_cast<int>(alpha_grd_ * (delta_upper - delta_width)));
            gradient = clip_lu(gradient);
        }

        upper = clip_lu(upper + delta_upper);
        width = clip_lu(width + delta_width);
