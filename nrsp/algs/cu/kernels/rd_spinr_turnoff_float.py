...
rd_spinr_turnoff_float_kernel = cp.RawKernel(
    """
//cuda


extern "C" __global__
void rd_spinr_turnoff_float_kernel(
    const float *x,              // [n_frames, n_chirps, n_samples, 2]
    const float *R,            // [n_distances, 2] (range phasor weights)
    const float *D,            // [n_velocities, 2] (doppler phasor weights)
    const int n_samples,
    const int n_chirps,
    const float *params,       //
    float *grad,                 // [n_frames, n_distances, n_velocities]
    ...
    for (int chirp = 0; chirp < n_chirps; ++chirp) {
        // Chirp rotation
        float re_0 = D[2 * doppler_idx + 0] * s[0];
        float re_1 = D[2 * doppler_idx + 1] * s[1];
        float im_0 = D[2 * doppler_idx + 1] * s[0];
        float im_1 = D[2 * doppler_idx + 0] * s[1];
        s[0] = re_0 - re_1;
        s[1] = im_0 + im_1;
        ...
        for (int t = 0; t < n_samples; ++t) {
            ...
            // Sample rotation
            float re_0 = R[2 * distance_idx + 0] * s[0];
            float re_1 = R[2 * distance_idx + 1] * s[1];
            float im_0 = R[2 * distance_idx + 1] * s[0];
            float im_1 = R[2 * distance_idx + 0] * s[1];
            s[0] = re_0 - re_1;
            s[1] = im_0 + im_1;
            ...
            if (silent_chirp == thresh_silent_chirp) {
                // only count neurons that turnoff due to turnoff logic and not spiking here
            ...
            if (sample_idx > t_grd) {
                ...
                // gradient = gradient > 0 ? gradient : 0; // ReLU
            }
...
""",
    name="rd_spinr_turnoff_float_kernel",
)
