...
class Cu_RA_SpiNR_RateCaCfar_NX_Model:
    ...
    def _forward_cfar(self, spikes, cfar_out):
        ...
        N = calculate_num_ref_cells(self.guard_cells, self.ref_cells)
        nx_scale = cp.asarray([find_best_ca_cfar_weight_scale_nx(a, N) for a in self.alpha_cfar])

        ref_weights_nx = cp.rint(cp.asarray(self.alpha_cfar, dtype=cp.float32) / N * nx_scale)
        alphas_nx = ref_weights_nx * N / cp.rint(nx_scale)
        alphas_nx = cp.array(alphas_nx, cp.float32)

        offsets_nx = cp.rint(cp.asarray(self.offset_cfar), dtype=cp.float32)  # TOOD
        ...
        rate_ca_cfar_kernel(
            ...
            (
                ...
                self.wrap_y,
                alphas_nx,
                cp.int32(alphas_nx.size),
                offsets_nx,
                cp.int32(offsets_nx.size),
                cfar_out,
            ),
        )
