...
from nrsp.algs.cu.kernels.rd_spinr_turnoff_v2_nx import rd_spinr_turnoff_v2_nx_kernel
from nrsp.algs.cu.kernels.rd_spinr_turnoff_nx import rd_spinr_turnoff_nx_kernel
...
class Cu_RD_SpiNR_LogOsCfar_NX_Model:
    ...
    def __init__(
        ...
        # cupy init
        self.R = nrsp.utils.cu.complex_to_float(phasor_weights(self.n_samples)[self._range_bins])
        self.D = nrsp.utils.cu.complex_to_float(cp.fft.fftshift(phasor_weights(self.n_chirps)))

        self._grad = cp.zeros((self.n_frames, self._n_distances, self._n_velocities), dtype=cp.int32)
    ...
    def _spinr_params(self):
        ...
        spinr_params[0] = cp.int32(self.alpha_grd * 2**15)
    ...
    def _forward_spinr(self, x):
        kernel = rd_spinr_turnoff_v2_nx_kernel if self.kernel_version == "v2" else rd_spinr_turnoff_nx_kernel
        kernel(
