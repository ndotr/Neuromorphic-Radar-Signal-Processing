...
class NxRSpiNRModel():
    ...
    Architecture:
    (1) channel are fed to (n_neurons_0) neurons with different dynamics.
   ...
   (1)      (n_neurons_0)               (n_neurons_0) 
    ...
    def __init__(self, n_neurons,
        ...
        #self.weights = nrsp.utils.nx.complex_to_float(np.ones((self.n_neurons, self.n_channels)))
        self.weights = np.ones((self.n_neurons, self.n_channels, 2))
        ...
        self.spinr_ucode_args = {
                                'alpha_mag': (alpha_mag*2**8),
                                'beta_mag': ((1 - alpha_mag)*2**10),
    ...
    def _build_spinr_synapses(self):
        ...
        synapses = [K.Linear(weight=self.weights[...,0], **syn_args, name='r2r_wgt'), 
                    K.Linear(weight=self.weights[...,1], **syn_args, name='i2i_wgt'),
                   ]
    ...
    def forward_cpu(self, input_data):
        ...
        assert self.n_timesteps == input_data.shape[0]
        output = np.zeros((self.n_timesteps, self.n_neurons))
        try:
            ...
            #self.board.fetchAll()
            #grad = self.model.spinr.neuron.grd.get(self.board)
            print('Done.')
